...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const existing = await admin.rpc("get_user_id_by_email", { p_email: norm });
    let created = false;
    if (!existing.data) {
      ...
      if (c.error) {
        ...
        const again = await admin.rpc("get_user_id_by_email", { p_email: norm });
        if (!again.data) throw c.error;
      } else {
        created = true;
      }
    }
