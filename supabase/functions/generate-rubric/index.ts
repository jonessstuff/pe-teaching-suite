...
import { buildRubricPrompt } from "../_shared/rubricPrompt.js";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { system, user } = buildRubricPrompt(row.lesson_object);
    const result = await callClaudeForJson(system, user, 6000);

    if (!result?.rubric) return errorResponse("Model response missing rubric", 500);
    return jsonResponse({ rubric: result.rubric });
  } catch (err) {
    console.error(err);
