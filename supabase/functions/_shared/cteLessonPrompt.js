...
Content areas to prioritize: (a) Web Design fundamentals — HTML/CSS basics, website structure & layout, user-experience/design principles, and responsive-design concepts; (b) Web Design tools & workflow — introduce design/development tools in an age-appropriate, TOOL-AGNOSTIC way wherever possible (specific software varies by school), and the plan → build → test → revise project workflow; (c) IT foundations & CYBERSECURITY — basic computing concepts, digital literacy, and how the internet and web work, PLUS a deep, dedicated cybersecurity strand (CIA triad and threat types, network-security concepts, digital ethics / law / responsible disclosure, and the A+ → Network+ → Security+ certification pathway — see the CYBERSECURITY CONTENT block above); at the MS Exploratory level keep cybersecurity awareness-level and conceptual; and (d) career connections — IT / web-design / cybersecurity career paths, portfolio-building concepts, and the industry certifications students can pursue (CompTIA A+/Network+/Security+). Keep tool references generalizable across schools rather than tied to one product.`,
...
function getCredentialFocus(pathway) {
  return {
    ...
    information_technology: ["CompTIA certification pathway — the real, sequential industry progression: IT Fundamentals+ (ITF+) on-ramp → A+ → Network+ → Security+ (Security+ is the recognized entry-level CYBERSECURITY credential); achievable, industry-recognized credentials students can genuinely pursue", "Web-design / development certificates & micro-credentials (e.g., Responsive Web Design, HTML & CSS)", "FBLA Website Design / Introduction to Programming / Cybersecurity competitive event recognition"],
...
function getPathwaySequence(pathway) {
  return {
    ...
    information_technology: [
      ...
      { level: "concentrator", course: "Web Design & Development / IT & Cybersecurity Concentration", description: "Applied web design & development — deeper HTML/CSS, responsive and user-centered design, the plan→build→test→revise workflow and design tools — PLUS a dedicated cybersecurity strand (CIA triad and threats, network-security concepts, digital ethics & responsible disclosure) building toward the CompTIA A+ → Network+ → Security+ pathway, aligned to NICE work roles. Defensive and conceptual." },
