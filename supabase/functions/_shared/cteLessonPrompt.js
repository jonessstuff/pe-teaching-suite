...
- AP Computer Science Principles (College Board) — a real, achievable advanced course anchor for the COMPUTER PROGRAMMING area: a multidisciplinary introduction to the big ideas of computing (creative development; data; algorithms & programming; computer systems & networks; and the impact of computing). Framework field: "AP CSP".
- AP Computer Science A (College Board) — the traditional programming-language-focused advanced course (typically Java-based) for deeper programming study. Framework field: "AP CSA". NOTE both AP CSP and AP CSA offer real college-credit potential and are widely available through free, standards-aligned curricula (e.g., Code.org, CodeHS) — present them as concrete, achievable student goals, not aspirational asides.
...
- OFFICE PRODUCTIVITY & AI TOOLS — practical, PRODUCT-NEUTRAL business-software proficiency at a concepts/skills level: WORD PROCESSING (professional documents, formatting), PRESENTATIONS (clear, well-designed slides and effective delivery), and SPREADSHEETS (data entry, basic formulas, simple charts, organizing and analyzing data) — teach the transferable SKILLS, not one specific product or version. Plus responsible, practical use of AI TOOLS in a business context: where AI genuinely helps productivity (drafting, summarizing, brainstorming, organizing information), its LIMITS and risks (accuracy / "hallucinations", data privacy, bias), and the ETHICS of appropriate use (disclosure, human review/verification, and academic/workplace integrity). Frame as general business-education / digital-literacy best practice; tie to Microsoft Office Specialist (MOS) / administrative-skills or FBLA computer-application events where they genuinely map.`,
...
function getCredentialFocus(pathway) {
  return {
    ...
    health_science: ["NHA certifications (e.g., CCMA Clinical Medical Assistant, CPT Phlebotomy Technician)", "DANB (Dental Assisting National Board) for dental-assisting lessons — CDA (Certified Dental Assistant), earned via three component exams: General Chairside (GC), Radiation Health & Safety (RHS), and Infection Control (ICE); plus NELDA (National Entry Level Dental Assistant), designed to be attainable through high-school dental-assisting coursework", "CPR / First Aid certification (AHA or American Red Cross)", "HOSA competitive event & program recognition"],
    education: ["Educators Rising Micro-credentials & competitive event recognition", "Teacher Cadet Program completion & articulated / dual college credit (CERRA 'Experiencing Education')", "ETS ParaPro Assessment (entry-level paraeducator credential)", "Child Development Associate (CDA) credential — for early-childhood / education field placements"],
