...
import { buildDancePrompt } from "../_shared/dancePrompt.js"
...
Deno.serve(async (req: Request) => {
  console.log("[generate-dance] handler entered, method:", req.method)
  ...
  try {
    const { system, user } = buildDancePrompt(body as Record<string, unknown>)

    // Prose-dense NCCAS lesson (objectives, warm-up, 2–3 activities, safety, standards).
    ...
    await captureLessonGenerated(req, { subject: "Dance", grades: gradeBand ? [gradeBand] : [], type: "dance", durationMs: Date.now() - _t0 });
    return jsonResponse(result)
  } catch (err) {
    await reportError(err, { fn: "generate-dance" });
    console.error("[generate-dance] error:", err)
