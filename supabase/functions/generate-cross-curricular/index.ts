...
import { buildCrossCurricularPrompt } from "../_shared/crossCurricularPrompt.js";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { system, user } = buildCrossCurricularPrompt(row.lesson_object);
    const result = await callClaudeForJson(system, user, 6000);

    if (!Array.isArray(result?.connections)) return errorResponse("Model response missing connections", 500);
    return jsonResponse({ connections: result.connections });
  } catch (err) {
    console.error(err);
