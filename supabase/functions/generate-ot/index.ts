...
import { buildOtPrompt } from "../_shared/otPrompt.js"
...
const VALID_AREAS = ["fine_motor", "sensory", "adls", "visual_motor", "vocational"]

Deno.serve(async (req: Request) => {
  console.log("[generate-ot] handler entered, method:", req.method)
  ...
  try {
    const { system, user } = buildOtPrompt(body as Record<string, unknown>)
    ...
    await captureLessonGenerated(req, { subject: "Occupational Therapists", grades: gradeBand ? [gradeBand] : [], type: "ot", durationMs: Date.now() - _t0 });
    return jsonResponse(result)
  } catch (err) {
    await reportError(err, { fn: "generate-ot" });
    console.error("[generate-ot] error:", err)
