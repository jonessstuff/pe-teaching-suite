...
export default function TheaterRenderer({ lesson }) {
    ...
    <div className="card max-w-3xl mx-auto p-8 space-y-6">
      ...
      <header className="space-y-2 border-b border-ink-900 pb-4">
        <div className="flex flex-wrap items-center gap-2">
          <span className="label-eyebrow rounded px-2 py-0.5 bg-maroon-500/15 text-ink-50">
            Theater · {lesson.artistic_process || 'Artistic Process'}
            {bands.length > 0 ? ` · Grade${bands.length > 1 ? 's' : ''} ${bands.map(formatGrade).join('/')}` : ''}
          </span>
          {lesson.band_label && (
            <span className="rounded px-2 py-0.5 text-xs font-semibold bg-maroon-500/20 text-ink-50">{lesson.band_label}</span>
          )}
          {lesson.hs_tier && (
      ...
      {(lesson.enduring_understanding || lesson.essential_question) && (
        <div className="grid gap-3 sm:grid-cols-2">
          {lesson.enduring_understanding && (
            <div className="rounded-lg border border-maroon-500/25 bg-maroon-500/5 px-4 py-3">
              <p className="text-xs font-semibold text-maroon-400 mb-0.5">Enduring understanding</p>
          ...
          <ul className="space-y-1 text-ink-300">
            {objectives.map((o, i) => (
              <li key={i} className="flex items-start gap-2 text-sm">
                <Target size={14} className="mt-0.5 shrink-0 text-maroon-400" />
                <span>{o}</span>
              </li>
            ))}
          </ul>
            ...
            {activities.map((a, i) => (
              <div key={i} className="rounded-lg bg-ink-900 px-4 py-3">
                <div className="mb-1 flex items-center gap-2">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-maroon-500/25 text-xs font-semibold text-ink-50">{i + 1}</span>
              ...
              <tbody>
                {vocab.map((v, i) => (
                  <tr key={i} className={i % 2 ? 'bg-ink-900/40' : ''}>
                    <td className="px-3 py-1.5 font-medium text-maroon-300 align-top whitespace-nowrap">{v.term}</td>
          ...
          <ul className="space-y-1.5 text-ink-300">
            {(lesson.standards_alignment ?? []).map((s, i) => (
              <li key={i} className="text-sm">
                <span className="rounded bg-maroon-500/15 px-1.5 py-0.5 text-xs font-semibold text-ink-50">{s.framework}{s.code ? ` ${s.code}` : ''}</span>
      ...
      {/* Copyright note — always prominent */}
      {lesson.copyright_note && (
        <div className="flex items-start gap-2.5 rounded-lg border border-maroon-500/25 bg-maroon-500/5 px-4 py-3">
          <ShieldAlert size={15} className="mt-0.5 shrink-0 text-maroon-400" />
          <p className="text-xs text-ink-300"><span className="font-medium text-ink-300">Scripts &amp; copyright: </span>{lesson.copyright_note}</p>
        </div>
      )}
      ...
      <div className="flex items-center justify-between border-b border-ink-900 pb-1">
        ...
        {copyText && (
          <button
            type="button"
            onClick={handleCopy}
            className="flex items-center gap-1 text-xs text-ink-400 hover:text-maroon-300 transition-colors print:hidden"
