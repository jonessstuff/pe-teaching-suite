...
export default function ArtPlanRenderer({ lesson }) {
  ...
  return (
    <div className="card max-w-3xl mx-auto p-8 space-y-6">
          ...
          <GradeBandBlock key={`standards-${grade}`} grade={grade}>
            <ul className="list-disc list-inside space-y-1">
              {(lesson.standards ?? [])
                ...
                .map((s, i) => (
                  <li key={i}>
                    <span className="font-semibold text-orange-400">{s.code}</span>
                    {' — '}
                    {s.text}
                  </li>
                ))}
      ...
      <Section title="Lesson Flow" copyText={instructionText}>
        <InstructionBlock label="Introduction & Inspiration" text={lesson.warm_up} />
        <InstructionBlock label="Teacher Demonstration" text={lesson.whole_group_instruction} />
        <InstructionBlock label="Guided / Shared Practice" text={lesson.fitness_activities} />
        <InstructionBlock label="Independent Creation" text={lesson.independent_practice} />
        <InstructionBlock label="Cleanup & Reflection" text={lesson.closure} />
      </Section>
      ...
      {/* Supplies */}
      <Section title="Supplies & Setup" copyText={suppliesText}>
        <div className="space-y-3">
          <div>
            <p className="text-sm font-semibold text-ink-200 mb-1">Supplies</p>
          ...
          <div>
            <p className="text-sm font-semibold text-ink-200 mb-1">Art Room Setup</p>
      ...
      {([...(lesson.routines ?? []), ...(lesson.behavior_notes ?? [])]).length > 0 && (
        <Section title="Art Room Routines & Studio Expectations" copyText={routinesText}>
          <ul className="list-disc list-inside space-y-1 text-ink-300">
      ...
      {(lesson.safety_notes ?? []).length > 0 && (
        <Section title="Material Safety" copyText={safetyText}>
          <ul className="list-disc list-inside space-y-1 text-ink-300">
      ...
      <div className="flex items-center justify-between border-b border-ink-900 pb-1">
        ...
        {copyText && (
          <button
            type="button"
            onClick={handleCopy}
            className="flex items-center gap-1 text-xs text-ink-400 hover:text-orange-400 transition-colors print:hidden"
