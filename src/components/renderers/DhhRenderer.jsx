...
export default function DhhRenderer({ lesson }) {
    ...
    <div className="card lesson-doc p-8 space-y-6">
      ...
      <header className="space-y-2 border-b border-ink-900 pb-4">
        <div className="flex flex-wrap items-center gap-2">
          <span className="label-eyebrow rounded px-2 py-0.5 bg-magenta-500/15 text-ink-50">
            D/HH · {lesson.content_area || 'ECC-DHH Activity'}
            {bands.length > 0 ? ` · Grade${bands.length > 1 ? 's' : ''} ${bands.map(formatGrade).join('/')}` : ''}
          </span>
          {lesson.band_label && (
            <span className="rounded px-2 py-0.5 text-xs font-semibold bg-magenta-500/20 text-ink-50">{lesson.band_label}</span>
          )}
          {lesson.communication_approach && (
            <span className="rounded px-2 py-0.5 text-xs font-semibold bg-ink-800 text-ink-200">{lesson.communication_approach}</span>
          )}
        </div>
            ...
            {activities.map((a, i) => (
              <div key={i} className="rounded-lg bg-ink-900 px-4 py-3">
                <div className="mb-1 flex items-center gap-2">
                  <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-magenta-500/25 text-xs font-semibold text-ink-50">{i + 1}</span>
          ...
          <div className="space-y-1.5">
            {grading.map((g, i) => (
              <div key={i} className="flex items-start gap-2 text-sm text-ink-300">
                <SlidersHorizontal size={14} className="mt-0.5 shrink-0 text-magenta-400" />
      ...
      {lesson.age_dignity_note && (
        <div className="rounded-lg border border-magenta-500/25 bg-magenta-500/5 px-4 py-3">
          ...
          <ul className="space-y-1.5 text-ink-300">
            {(lesson.standards_alignment ?? []).map((s, i) => (
              <li key={i} className="text-sm">
                <span className="rounded bg-magenta-500/15 px-1.5 py-0.5 text-xs font-semibold text-ink-50">{s.framework}</span>
      ...
      {/* Professional boundary — always prominent */}
      {lesson.clinical_boundary_note && (
        <div className="flex items-start gap-2.5 rounded-lg border border-amber-500/25 bg-amber-500/5 px-4 py-3">
          ...
          <p className="text-xs text-ink-300"><span className="font-medium text-ink-300">Professional boundary: </span>{lesson.clinical_boundary_note}</p>
        </div>
      )}
      ...
      <div className="flex items-center justify-between border-b border-ink-900 pb-1">
        ...
        {copyText && (
          <button
            type="button"
            onClick={handleCopy}
            className="flex items-center gap-1 text-xs text-ink-400 hover:text-magenta-300 transition-colors print:hidden"
