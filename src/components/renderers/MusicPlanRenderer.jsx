...
export default function MusicPlanRenderer({ lesson }) {
  ...
  return (
    <div className="card max-w-3xl mx-auto p-8 space-y-6">
          ...
          <GradeBandBlock key={`standards-${grade}`} grade={grade}>
            <ul className="list-disc list-inside space-y-1">
              {(lesson.standards ?? [])
                ...
                .map((s, i) => (
                  <li key={i}>
                    <span className="font-semibold text-purple-400">{s.code}</span>
                    {' — '}
                    {s.text}
                  </li>
                ))}
      ...
      <Section title="Lesson Flow" copyText={instructionText}>
        ...
        <InstructionBlock label="Concept Introduction" text={lesson.whole_group_instruction} />
        <InstructionBlock label="Listening Example" text={lesson.fitness_activities} />
        <InstructionBlock label="Active Music Making" text={lesson.independent_practice} />
        <InstructionBlock label="Assessment & Reflection" text={lesson.closure} />
      </Section>
      ...
      {/* Instruments & Setup */}
      <Section title="Instruments & Setup" copyText={instrumentsText}>
        <div className="space-y-3">
          <div>
            <p className="text-sm font-semibold text-ink-200 mb-1">Instruments & Materials</p>
          ...
          {(lesson.equipment_alternatives ?? []).length > 0 && (
            <div>
              <p className="text-sm font-semibold text-ink-200 mb-1">Lower-resource alternatives</p>
              <ul className="list-disc list-inside space-y-1 text-ink-300">
          ...
          <div>
            <p className="text-sm font-semibold text-ink-200 mb-1">Music Room Setup</p>
      ...
      {([...(lesson.routines ?? []), ...(lesson.behavior_notes ?? [])]).length > 0 && (
        <Section title="Music Room Routines & Expectations" copyText={routinesText}>
          <ul className="list-disc list-inside space-y-1 text-ink-300">
      ...
      {(lesson.safety_notes ?? []).length > 0 && (
        <Section title="Instrument Safety" copyText={safetyText}>
          <ul className="list-disc list-inside space-y-1 text-ink-300">
      ...
      <div className="flex items-center justify-between border-b border-ink-900 pb-1">
        ...
        {copyText && (
          <button
            type="button"
            onClick={handleCopy}
            className="flex items-center gap-1 text-xs text-ink-400 hover:text-purple-400 transition-colors print:hidden"
