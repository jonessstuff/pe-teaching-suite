...
export default function GettingStartedChecklist() {
  ...
  const isNewAccount = profile?.created_at && mountedAt - new Date(profile.created_at).getTime() <= 14 * 86400000

  useEffect(() => {
    if (!loaded || !profile || !isHome || dismissed || !isNewAccount) return undefined
    let active = true
    Promise.all([
    ...
    return () => { active = false }
  }, [dismissed, isHome, isNewAccount, loaded, profile])

  const steps = useMemo(() => counts ? [
    { label: 'Tell us what you teach', done: !!profile?.onboarded_at || !!profile?.teaching_areas?.length, to: '/settings' },
    { label: 'Create your first ready-to-teach lesson', done: counts.lessons >= 1, to: '/generate' },
    { label: 'Add a class', done: counts.periods >= 1, to: '/students' },
    { label: 'Add or import a roster', done: counts.students >= 1, to: '/students' },
    { label: 'Create a second lesson to build your library', done: counts.lessons >= 2, to: '/generate' },
  ] : [], [counts, profile])

  if (!isHome || !isNewAccount || dismissed || !counts) return null
  ...
  return <section className="mb-6 overflow-hidden rounded-2xl border border-accent-500/25 bg-gradient-to-br from-accent-500/10 via-white to-violet-500/5 p-5 dark:via-ink-900 dark:to-violet-500/10">
    <div className="flex items-start justify-between gap-4">
      ...
      <button aria-label="Dismiss checklist" className="rounded-lg p-1.5 text-ink-500 hover:bg-ink-800 hover:text-ink-200" onClick={() => { localStorage.setItem(DISMISSED_KEY, 'true'); setDismissed(true) }}><X size={17} /></button>
    </div>
