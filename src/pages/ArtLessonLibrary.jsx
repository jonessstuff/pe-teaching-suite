...
export default function ArtLessonLibrary() {
  ...
  function matchesSearch(l) {
    ...
    const stageLabel = (l.lesson_object?.stage_label ?? '').toLowerCase()
    return title.includes(searchTerm) || unit.includes(searchTerm) || stageLabel.includes(searchTerm)
  }
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="label-eyebrow mb-2">Art</p>
          <h1 className="text-2xl font-semibold">Lesson Archive</h1>
        </div>
        <Link to="/art/generate" className="btn-primary">
          <Sparkles size={16} />
          New lesson
        </Link>
      </div>
      ...
      {lessons !== null && filtered.length === 0 && (
        <div className="card p-8 text-center">
          <p className="text-ink-500">
            {lessons.length === 0 ? 'No art lessons saved yet.' : 'No lessons match your search.'}
          </p>
          {lessons.length === 0 && (
            <Link to="/art/generate" className="btn-primary mt-4 inline-flex">
              <Sparkles size={16} />
              Generate your first art lesson
            </Link>
          )}
        </div>
      )}
