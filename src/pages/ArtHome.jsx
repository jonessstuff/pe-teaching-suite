...
import { Palette, Sparkles, BookOpen, BookMarked, CalendarDays, ClipboardList, Layers, ArrowRight, ArrowLeft, BarChart3, CalendarRange, FileInput, ScrollText, PartyPopper, Flame, BookCheck } from 'lucide-react'
  ...
  useEffect(() => {
    listLessons()
      .then((all) => setLessons(all.filter((l) =>
        (l.lesson_object?.subject ?? l.subject) === 'Art'
      )))
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          <div className="flex items-start justify-between">
            ...
            <ArrowRight
              size={18}
              className="mt-0.5 text-ink-700 transition-transform group-hover:translate-x-1 group-hover:text-orange-400"
            />
          </div>
          <div>
            <p className="font-semibold text-ink-50">Generate an art lesson</p>
            <p className="mt-0.5 text-sm text-ink-500">
              7-phase studio lesson — demo, guided practice, and independent creation
            </p>
          </div>
        </Link>
        ...
        >
          ...
          <div>
            ...
            <p className="mt-0.5 text-sm text-ink-500">
              2–3 connected studio stages built around one art project
            </p>
          </div>
        </Link>
        ...
        <Link to="/assessments?module=Art" className="card group flex flex-col gap-4 p-6 transition-colors hover:border-indigo-500/40">
          <div className="flex items-start justify-between">
        ...
        <Link to="/standards-tracker?module=Art" className="card group flex flex-col gap-4 p-6 transition-colors hover:border-violet-500/40">
          <div className="flex items-start justify-between">
        ...
        <Link to="/pacing-guide?module=Art" className="card group flex flex-col gap-4 p-6 transition-colors hover:border-teal-500/40">
          <div className="flex items-start justify-between">
        ...
        <Link to="/import?subject=art&amp;module=Art" className="card group flex flex-col gap-4 p-6 transition-colors hover:border-rose-500/40">
          <div className="flex items-start justify-between">
        ...
        <Link to="/eoy-narrative?module=Art" className="card group flex flex-col gap-4 p-6 transition-colors hover:border-sky-500/40">
          <div className="flex items-start justify-between">
        ...
        <Link to="/activity-bank?module=Art" className="card group flex flex-col gap-4 p-6 transition-colors hover:border-yellow-500/40">
          <div className="flex items-start justify-between">
        ...
        <Link to="/warm-up-generator?module=Art" className="card group flex flex-col gap-4 p-6 transition-colors hover:border-orange-500/40">
          <div className="flex items-start justify-between">
