...
function CustomerWorkspace({ customers, filter, setFilter, onRefresh }) {
  ...
  function exportCsv() {
    const header = ['Name', 'Email', 'Status', 'Access ends', 'Canceled', 'Lessons', 'Inactive days', 'Joined', 'Last activity', 'Teaching areas', 'Cancellation reason', 'Feedback', 'Last contacted', 'Follow-up', 'Outcome', 'Owner note']
    const rows = visible.map((c) => [c.name, c.email, c.status, c.accessEndsAt, c.canceledAt, c.lessonCount, c.inactiveDays, c.joinedAt, c.lastActivityAt, (c.teachingAreas ?? []).join('; '), REASON_LABELS[c.cancellationFeedback?.reason] ?? c.cancellationFeedback?.reason, c.cancellationFeedback?.detail, c.contact?.last_contacted_at, c.contact?.follow_up_at, c.contact?.outcome, c.contact?.note])
