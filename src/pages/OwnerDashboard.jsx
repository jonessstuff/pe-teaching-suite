...
import { BarChart3, CreditCard, DollarSign, Eye, Loader2, MousePointerClick, RefreshCw, TrendingUp, Users, XCircle } from 'lucide-react'
...
export default function OwnerDashboard() {
  ...
  return <div className="space-y-7">
    ...
    <section><h2 className="mb-3 font-semibold text-ink-200">Revenue health</h2><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5"><Metric icon={CreditCard} label="Paying subscribers" value={s.active} color="text-emerald-400" /><Metric icon={XCircle} label="Canceling" value={s.canceling} note="Ends after current period" color="text-red-400" /><Metric icon={Users} label="Trials" value={s.trialing} color="text-violet-400" /><Metric icon={DollarSign} label="Estimated MRR" value={`$${(s.mrrCents / 100).toFixed(0)}`} note="Annual plans normalized" color="text-amber-400" /><Metric icon={TrendingUp} label="Current access" value={s.current} note="Paid + trialing" /></div></section>
