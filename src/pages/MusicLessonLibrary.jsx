...
export default function MusicLessonLibrary() {
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="label-eyebrow mb-2">Music</p>
          <h1 className="text-2xl font-semibold">Lesson Archive</h1>
        </div>
        <Link to="/music/generate" className="btn-primary">
          <Sparkles size={16} />
          New lesson
        </Link>
      </div>
      ...
      {lessons !== null && filtered.length === 0 && (
        <div className="card p-8 text-center">
          <p className="text-ink-500">
            {lessons.length === 0 ? 'No music lessons saved yet.' : 'No lessons match your search.'}
          </p>
          {lessons.length === 0 && (
            <Link to="/music/generate" className="btn-primary mt-4 inline-flex">
              <Sparkles size={16} />
              Generate your first music lesson
            </Link>
          )}
        </div>
      )}
      ...
      {sorted.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {sorted.map((lesson) => (
            <LessonCard key={lesson.id} lesson={lesson} moduleContext="Music" />
          ))}
        </div>
      )}
    </div>
