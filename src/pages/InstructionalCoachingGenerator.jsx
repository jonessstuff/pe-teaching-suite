...
export default function InstructionalCoachingGenerator() {
    ...
    return (
      <div>
        <div className="mb-6 flex items-center gap-3 print:hidden">
          <Link to="/instructional-coaching" className="flex items-center gap-1.5 text-sm text-ink-400 hover:text-ink-200 transition-colors">
            <ArrowLeft size={16} /> Instructional Coaching
          </Link>
  ...
  return (
    <div className="max-w-2xl space-y-8">
      {/* Header */}
      <div>
        <Link to="/instructional-coaching" className="mb-3 flex items-center gap-1.5 text-sm text-ink-500 hover:text-ink-200 transition-colors">
          <ArrowLeft size={14} /> Instructional Coaching
        </Link>
      ...
      <form onSubmit={handleGenerate} className="space-y-6">
        ...
        {loading && (
          <p className="text-center text-xs text-ink-500">This usually takes 1–2 minutes · Do not close this tab</p>
        )}
      </form>
    </div>
  )
