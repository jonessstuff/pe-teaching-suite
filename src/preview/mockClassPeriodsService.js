export const previewPeriods = [
  ...
  { id: 'period-library', label: 'Grade 4 Library', subject: 'Library/Media', grade_bands: [4], class_size: 5, duration_minutes: 40 },
]

export async function listPeriods() { return previewPeriods.map((period) => ({ ...period, grade_bands: [...(period.grade_bands ?? [])] })) }
export async function createPeriod(values) { const row = { id: `period-${Date.now()}`, ...values }; previewPeriods.push(row); return { ...row, grade_bands: [...(row.grade_bands ?? [])] } }
